...
class LSHash(object):
    ...
    def query(self, query_point, num_results=None, distance_func=None):
        ...
        if distance_func == "hamming":
            ...
            for i, table in enumerate(self.hash_tables):
                ...
                for key in table.keys():
                    ...
                    if distance < 2:
                        candidates_list.extend(table.get_list(key))
            ...
            for i, table in enumerate(self.hash_tables):
                binary_hash = self._hash(self.uniform_planes[i], query_point)
                candidates_list.extend(table.get_list(binary_hash))
        ...
        return len(candidates_list), candidates[:num_results] if num_results else candidates
