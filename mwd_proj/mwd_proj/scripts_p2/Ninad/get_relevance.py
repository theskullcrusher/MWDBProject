...
def movie_rel_ratio_DF(mv_relevance):
    ...
    for mv in rel_mv:
        cur2.execute("SELECT genres FROM `mlmovies_clean` where movieid = %s", {mv})
    ...
    cur2.execute("SELECT Distinct(movieid) FROM `mlmovies_clean`")
    result_mv = cur2.fetchall()
    for mv_id in result_mv:
        cur2.execute("SELECT genres FROM `mlmovies_clean` where movieid = %s", {mv_id})
        result_gen = cur2.fetchall()
